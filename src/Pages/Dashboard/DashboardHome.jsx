...
const DashboardHome = () => {
  ...
  // Dummy data, replace with real queries if needed
   useEffect(() => {
      if (user?.email) {
  ...
  const totalUserPlants = 12; // fetch from DB for this user
  const totalAllPlants = 58;
    return (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-green-100 p-6 rounded-xl shadow flex items-center gap-4">
          ...
          <div>
            <p className="text-3xl font-bold text-green-800">{allPlants.length}</p>
        ...
        <div className="bg-green-100 p-6 rounded-xl shadow flex items-center gap-4">
          ...
          <div>
            <p className="text-3xl font-bold text-green-800">{Math.round((totalUserPlants / totalAllPlants) * 100)}%</p>
